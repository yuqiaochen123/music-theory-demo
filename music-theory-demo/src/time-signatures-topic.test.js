...
describe("Time signatures and grouping MVP",()=>{
  ...
  it("marks the downbeat at the start of every rhythmic group",()=>{
    ...
    for(const item of [...lesson,...practice]){
      ...
      item.events.forEach(event=>assert.equal(Boolean(event.accent),firstByGroup.get(event.group)===event));
    }
  });
});
