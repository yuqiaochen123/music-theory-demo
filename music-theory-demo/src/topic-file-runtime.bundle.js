...
${suffix}`;
  ...
  var SUPABASE_URL = "https://pwofphatgbkhhmjaaxgl.supabase.co";
  var SUPABASE_PUBLISHABLE_KEY = "sb_publishable_np6qR5e7sn_vIifUj1c7pA_HtpOjpD4";
  function buildClient(createClient2) {
    return createClient2(SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY, {
      auth: {
        persistSession: true,
        autoRefreshToken: true,
        detectSessionInUrl: true
  ...
  var browserClientFactory = globalThis.supabase?.createClient;
  var defaultClient = browserClientFactory ? buildClient(browserClientFactory) : null;
  async function getSupabaseClient() {
    if (defaultClient) return defaultClient;
    const { createClient: createClient2 } = await Promise.resolve().then(() => (init_dist4(), dist_exports));
    return buildClient(createClient2);
  }
  ...
  function createProgressStore({ client = defaultClient } = {}) {
    ...
    async function loadStudentData(studentId) {
      ...
      const db = await getClient();
      const { data: progress, error: progressError } = await db.from("student_progress").select("*").eq("student_id", resolvedStudentId).order("updated_at", { ascending: false });
      throwIfError(progressError, "Unable to load student progress");
      const { data: attempts, error: attemptsError } = await db.from("exercise_attempts").select("*").eq("student_id", resolvedStudentId).order("attempted_at", { ascending: false }).limit(100);
    ...
    async function findProgress(studentId, { grade, topicId, lessonId = null }) {
      const db = await getClient();
      let query = db.from("student_progress").select("id").eq("student_id", studentId).eq("grade", Number(grade)).eq("topic_id", topicId);
    ...
    async function deleteProgress({ grade, topicId, lessonId = null }) {
      const studentId = await initializeStudent();
      const db = await getClient();
      let query = db.from("student_progress").delete().eq("student_id", studentId).eq("grade", Number(grade)).eq("topic_id", topicId);
      query = lessonId === null ? query.is("lesson_id", null) : query.eq("lesson_id", lessonId);
  ...
  function sameLesson(record, { grade, topicId, lessonId = null }) {
    return Number(record.grade) === Number(grade) && record.topic_id === topicId && (record.lesson_id ?? null) === (lessonId ?? null);
  }
  ...
  async function startCurrentLesson({ grade = 5, topicId, lessonId = null, statusElement, syncElement } = {}) {
  ...
  function lessonExample(label, notes, lower, upper, explanation) {
    return {
      ...
      explanation,
      parts: [
        [`Lower note \xB7 ${lower.name}`, [lower.midi]],
        [`Upper note \xB7 ${upper.name}`, [upper.midi]]
      ]
    };
  }
  ...
  var INTERVAL_LESSONS = Object.freeze([
    ...
    intervalLesson("tritone", "Tritone", 6, "A tritone spans six semitones. It may be written as an augmented fourth or a diminished fifth.", [
      lessonExample("Augmented fourth \xB7 C to F-sharp", ["c/4", "f#/4"], { name: "C", midi: 60 }, { name: "F\u266F", midi: 66 }, "C to F-sharp is an augmented fourth."),
      lessonExample("Diminished fifth \xB7 C to G-flat", ["c/4", "gb/4"], { name: "C", midi: 60 }, { name: "G\u266D", midi: 66 }, "C to G-flat is a diminished fifth. It sounds like the augmented fourth beside it.")
    ]),
  ...
  var NOTE_DURATIONS = Object.freeze({ q: 4, "8": 2, "16": 1 });
  var MAJOR_KEYS = Object.freeze({
    C: ["c", "d", "e", "f", "g", "a", "b"],
    G: ["g", "a", "b", "c", "d", "e", "f#"],
    D: ["d", "e", "f#", "g", "a", "b", "c#"],
    A: ["a", "b", "c#", "d", "e", "f#", "g#"],
  ...
  var LETTER_SEMITONES = Object.freeze({ c: 0, d: 2, e: 4, f: 5, g: 7, a: 9, b: 11 });
  var LETTERS = Object.freeze(["c", "d", "e", "f", "g", "a", "b"]);
  var ACCIDENTAL_OFFSETS = Object.freeze({ bb: -2, b: -1, "": 0, "#": 1, "##": 2 });
  ...
  function applyAccidental(naturalPitch, accidental = "") {
  ...
  function createEditorState(initialNotes = ["c/4", "e/4", "g/4", "c/5"]) {
    ...
    return {
      notes: [...initialNotes],
      durations: initialNotes.map(() => "q"),
      barCount: 2,
      slots: initialNotes.map((_, index) => index * 4),
      selectedIndex: null,
  ...
  function withNotes(state, notes, slots, durations, message, cursorSlot = state.cursorSlot, selectedIndex = null) {
    return {
      ...
      selectedIndex,
      cursorSlot,
      history: [...state.history, historyEntry(state)],
      message,
      transposedNotes: []
    };
  }
  ...
  function canPlaceNote(state, slot, duration = "q") {
  ...
  function placeNote(state, pitch, slot, duration = "q") {
    requirePitch(pitch);
    requireDuration(duration);
    ...
    return withNotes(
      state,
      placements.map(({ note }) => note),
      placements.map(({ slot: value }) => value),
      placements.map(({ duration: value }) => value),
      `${pitchLabel(pitch)} ${durationLabel(duration)} placed.`,
      nextCursor,
      selectedIndex
    );
  }
  function placeAtCursor(state, pitch, duration = "q") {
    requireDuration(duration);
  ...
  function rhythmicRests(state) {
    ...
    for (let barStart = 0; barStart < state.barCount * 16; barStart += 16) {
      ...
      for (let slot = barStart; slot < barStart + 16; ) {
        ...
        const duration = slot % 4 === 0 && remaining >= 4 && [0, 1, 2, 3].every((offset) => !occupied.has(slot + offset)) ? "q" : slot % 2 === 0 && remaining >= 2 && [0, 1].every((offset) => !occupied.has(slot + offset)) ? "8" : "16";
  ...
  function addBars(state, count = 1) {
  ...
  function selectNote(state, index) {
  ...
  function undo(state) {
    ...
    return {
      ...
      cursorSlot: previous.cursorSlot ?? 0,
      selectedIndex: null,
      history: state.history.slice(0, -1),
      message: "Last phrase edit undone.",
      transposedNotes: []
    };
  }
  function clearPhrase(state) {
  ...
  function setTargetKey(state, key) {
  ...
  function transposePhrase(state) {
  ...
  function scaleSlotCenters(slotCenters, engravingWidth, viewportWidth) {
  ...
  function mountClefTranspositionEditor({ container, notation, play, mode = "key" }) {
    ...
    function render() {
      ...
      noteList.innerHTML = state.notes.map((pitch, index) => `<button type="button" data-select-note="${index}" aria-pressed="${index === state.selectedIndex}">${pitchLabel(pitch)} \xB7 ${durationLabel(state.durations[index])}</button>`).join("");
    }
    ...
    function updatePointerPreview(event) {
      ...
      head2.setAttribute("rx", "7.5");
      head2.setAttribute("ry", "5");
      head2.setAttribute("transform", `rotate(-12 ${x} ${y})`);
      head2.setAttribute("fill", selectedDuration === "w" ? "white" : "#1687d9");
      head2.setAttribute("stroke", "#1687d9");
      head2.setAttribute("stroke-width", selectedDuration === "w" ? "2.5" : "1");
      preview.append(head2);
    ...
    container.querySelectorAll("[data-duration]").forEach((button) => button.addEventListener("click", () => {
      selectedDuration = button.dataset.duration;
      render();
    }));
    container.querySelectorAll("[data-accidental]").forEach((button) => button.addEventListener("click", () => {
    ...
    addBarsButton.addEventListener("click", () => {
      ...
      const count2 = Number(requested);
      if (Number.isInteger(count2) && count2 > 0 && count2 <= 32) update(addBars(state, count2));
  ...
  if (window.location.protocol === "file:") {
    let activeExample = function(index) {
      const base = displayedData().examples[index];
      ...
      rhythmLoop = null;
    }, toggleRhythmLoop = function(index, button) {
      if (rhythmLoop?.button === button) {
        stopRhythmLoop();
        return;
      }
      stopRhythmLoop();
      const pattern = activeExample(index).parts[0][1], playBar = () => sound(pattern, 0, 0.22);
      playBar();
      rhythmLoop = { button, timer: setInterval(playBar, pattern.length * 0.22 * 1e3) };
      button.textContent = "\u23F8 Pause";
    }, playPart = function(index, part) {
      stopRhythmLoop();
      const item = activeExample(index), spread = item.audioType === "sequence" ? 0.24 : item.audioType === "rhythm" ? 0.22 : 0;
      sound(item.parts[part][1], 0, spread);
    }, playExample = function(index) {
      const item = activeExample(index), parts = item.parts;
      if (topic === "scale-degrees") {
        sound(parts[0][1].concat(parts[1][1].slice(1)), 0, 0.24);
      } else if (item.completeMidi) {
        sound(item.completeMidi, 0, 0.24);
      } else if (topic === "cadences") {
        sound(parts[0][1]);
        sound(parts[1][1], 0.9);
      } else if (item.audioType === "sequence") {
        sound(parts.flatMap((part) => part[1]), 0, 0.24);
      } else if (item.audioType === "rhythm") {
        sound(parts[0][1], 0, 0.22);
      } else sound(parts.flatMap((part) => part[1]));
    }, groupingControls = function(base, item, index) {
    ...
    }, keySideMarkup = function(side, index, kind) {
      const heading = kind === "major" ? "Major key" : "Relative minor";
      return `<section class="key-side"><p class="eyebrow">${heading}</p><h3>${side.name}</h3><p class="key-tonic">Tonic: ${side.tonic}</p><div class="key-scale-notation" id="key-scale-${index}-${kind}" aria-label="${side.name} scale"></div><div class="key-controls"><button data-key-side="${index}" data-key-kind="${kind}" data-key-action="scale">\u25B6 Hear scale</button></div></section>`;
    }, playKeySide = function(index, kind) {
      stopRhythmLoop();
      const side = current().examples[index][kind];
      sound(side.midis, 0, 0.24);
    }, compareKeyPair = function(index) {
      stopRhythmLoop();
      const pair = current().examples[index];
      sound(pair.major.midis, 0, 0.24);
      setTimeout(() => sound(pair.minor.midis, 0, 0.24), 2100);
    }, renderKeySignatureLesson = function(data) {
      $(".compare-head").hidden = true;
      $(".continue").hidden = true;
      $("#examples").className = "examples";
      $("#examples").innerHTML = data.examples.map((pair, index) => `<article class="key-pair"><div class="key-pair-grid">${keySideMarkup(pair.major, index, "major")}${keySideMarkup(pair.minor, index, "minor")}</div><div class="key-bridge">Same key signature \xB7 different tonic</div><p class="key-explanation">Same signature and pitch collection; a different tonic creates the major or minor home note.</p><button class="key-compare" data-key-compare="${index}">\u25B6 Compare both</button></article>`).join("") + `<section class="key-reference-wrap"><p class="eyebrow">Grade 5 reference</p><h2>All relative key pairs through six accidentals.</h2><div class="key-reference"><table><thead><tr><th>Key signature</th><th>Major key</th><th>Relative minor</th></tr></thead><tbody>${data.reference.map((row) => `<tr><td>${row.signature}</td><td>${row.major}</td><td>${row.minor}</td></tr>`).join("")}</tbody></table></div></section>`;
      data.examples.forEach((pair, index) => ["major", "minor"].forEach((kind) => {
        const side = pair[kind], scale = document.querySelector(`#key-scale-${index}-${kind}`);
        try {
          ListeningDeskNotation.render(scale, { type: "scale", key: side.keySignature, notes: side.notes, showAccidentals: false }, { width: 360 });
        } catch (error) {
          scale.textContent = "Notation is unavailable in this browser.";
        }
      }));
      $("#examples").querySelectorAll("[data-key-side]").forEach((button) => button.onclick = () => playKeySide(+button.dataset.keySide, button.dataset.keyKind));
      $("#examples").querySelectorAll("[data-key-compare]").forEach((button) => button.onclick = () => compareKeyPair(+button.dataset.keyCompare));
      if (!(window.AudioContext || window.webkitAudioContext)) showAudioFallback();
    }, renderIntervalOverview = function() {
      const data = current();
      $("#lesson-topbar-title").textContent = "Intervals";
      $("#eyebrow").textContent = "Intervals";
      $("#title").textContent = "Learn every simple interval";
      $("#subtitle").textContent = "Choose an interval to see two written examples and hear it in the same listening format.";
      $("#intro").textContent = "Simple intervals fit within an octave. Each lesson shows correct spelling, the number of semitones, and matching notation and sound.";
      $("#footer").textContent = "Intervals";
      $(".compare-head").hidden = true;
      $(".continue").hidden = true;
      $("#examples").className = "examples interval-overview";
      $("#examples").innerHTML = INTERVAL_LESSONS.map((lesson) => `<a class="interval-link" href="topic.html?topic=intervals&lesson=${lesson.id}"><b>${lesson.label}</b><span>${lesson.semitones} semitone${lesson.semitones === 1 ? "" : "s"} \xB7 Learn \u2192</span></a>`).join("");
    }, conceptMarkup = function(concept) {
      return `<div class="concept-display"><div class="concept-symbol">${concept.symbol}</div><p class="concept-kicker">${concept.kicker}</p><p class="concept-detail">${concept.detail}</p></div>`;
    }, renderRhythmGuide = function(items) {
      const reference = $(".lesson-reference"), practice = $(".continue");
      document.querySelectorAll(".rhythm-guide-slide").forEach((slide) => slide.remove());
      reference.hidden = true;
      items.forEach((item, index) => {
        const slide = document.createElement("section");
        slide.className = "lesson-slide rhythm-guide-slide";
        slide.innerHTML = `<article class="rhythm-guide-card"><header><p class="eyebrow">Rhythm guide \xB7 ${index + 1} of ${items.length}</p><h2>${item.label}</h2><p class="rhythm-rule">${item.rule}</p></header><div class="rhythm-notation" id="rhythm-guide-notation-${index}" aria-label="${item.label} staff notation"></div><div class="rhythm-copy${item.detail ? "" : " rhythm-copy-single"}">${item.detail ? `<p class="rhythm-count">${item.detail}</p>` : ""}<p>${item.explanation}</p></div></article>`;
        practice.before(slide);
      });
      items.forEach((item, index) => {
        const target = document.querySelector(`#rhythm-guide-notation-${index}`);
        try {
          ListeningDeskNotation.render(target, item.notation, { width: 900 });
        } catch (error) {
          console.error(`Unable to render ${item.label} notation`, error);
          target.textContent = "Notation is unavailable in this browser.";
        }
      });
    }, renderLessonGuide = function(items) {
      const reference = $(".lesson-reference"), practice = $(".continue");
      ...
      items.forEach((item, index) => {
        ...
        const target = document.querySelector(`#lesson-guide-notation-${index}`);
        try {
          ListeningDeskNotation.render(target, item.notation, { width: 900 });
        } catch (error) {
          console.error(`Unable to render ${item.label} notation`, error);
      ...
      document.querySelectorAll("[data-guide-part]").forEach((button) => button.onclick = () => playPart(+button.dataset.guideIndex, +button.dataset.guidePart));
    }, render = function() {
      if (isIntervalOverview) {
      ...
      $("#lesson-topbar-title").textContent = intervalLesson2 ? intervalLesson2.label : data.name;
      $("#eyebrow").textContent = data.name;
      ...
      $("#examples").className = "examples" + (conceptOnly ? " concept-only" : "");
      $("#examples").innerHTML = items.map((item, index) => `<article class="example"><header><p>Example ${index + 1}</p><h3>${item.label}</h3></header>${groupingControls(data.examples[index], item, index)}${item.concept ? conceptMarkup(item.concept) : topic === "scale-degrees" ? scaleDegreeNotation(item, index) : `<div class="notation" id="notation-${index}" aria-label="${item.label} notation"></div>`}<div class="example-body"><p>${item.explanation}</p>${item.parts?.length >= 2 ? `<div class="play-row"><button data-index="${index}" data-part="0">\u25B6 ${item.parts[0][0]}</button><button data-index="${index}" data-part="1">\u25B6 ${item.parts[1][0]}</button><button data-index="${index}" data-part="2">\u25B6 ${topic === "time-signatures" ? "Hear multiple bars" : "Hear together"}</button></div>` : ""}</div></article>`).join("");
      items.forEach((item, index) => {
        if (item.concept) return;
        if (topic === "scale-degrees") {
          renderScaleDegreeNotation(item, index);
          return;
        }
        const target = document.querySelector(`#notation-${index}`);
        try {
          ListeningDeskNotation.render(target, item.notation, { width: 760 });
        } catch (error) {
          target.textContent = "Notation is unavailable in this browser.";
        }
      });
      $("#examples").querySelectorAll("[data-reveal-grouping]").forEach((button) => button.onclick = () => {
        const index = +button.dataset.revealGrouping;
        if (revealedGroupingSelectors.has(index)) revealedGroupingSelectors.delete(index);
        else revealedGroupingSelectors.add(index);
        render();
      });
      $("#examples").querySelectorAll("[data-grouping]").forEach((button) => button.onclick = () => {
        stopRhythmLoop();
        const index = +button.dataset.index;
        selectedVariants[index] = button.dataset.grouping;
        revealedGroupingSelectors.add(index);
        render();
      });
      $("#examples").querySelectorAll("[data-part]").forEach((button) => button.onclick = () => {
        const index = +button.dataset.index, part = +button.dataset.part;
        if (part === 2 && topic === "time-signatures") toggleRhythmLoop(index, button);
        else if (part === 2) {
          stopRhythmLoop();
          playExample(index);
      ...
      requestAnimationFrame(() => {
        ...
        dots.replaceChildren(...slides.map((slide, index) => {
          ...
          button.onclick = () => slide.scrollIntoView({ behavior: "smooth", inline: "center", block: "nearest" });
          return button;
        }));
        let frame = 0, lastSelected = -1, demoTimer = 0;
        const update = () => {
          frame = 0;
          ...
          slides.forEach((slide, index) => {
            ...
            const proximity = Math.max(0, 1 - next / carousel.clientWidth);
            const dot = dots.children[index];
            slide.style.setProperty("--slide-lift", `${(proximity * 14).toFixed(2)}px`);
            dot.style.setProperty("--dot-wave", proximity.toFixed(3));
            dot.style.setProperty("--dot-lift", `${(proximity * 4).toFixed(2)}px`);
            if (next < distance) {
              distance = next;
              selected = index;
            }
          });
          [...dots.children].forEach((dot, index) => dot.setAttribute("aria-current", index === selected ? "page" : "false"));
          if (selected !== lastSelected) {
            lastSelected = selected;
            clearTimeout(demoTimer);
            demoTimer = setTimeout(() => slides[selected]?.querySelector(".rhythm-notation")?.rhythmInteractionDemo?.play(), 380);
          }
        };
        const schedule = () => {
    ...
    let rhythmLoop = null;
    if (!isIntervalOverview) $("#compare").onclick = () => {
      stopRhythmLoop();
      playExample(0);
